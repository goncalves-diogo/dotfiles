...
_provision_completion() {
    local -a completions
    local -a completions_with_descriptions
    local -a response
    (( ! $+commands[provision] )) && return 1

    response=("${(@f)$(env COMP_WORDS="${words[*]}" COMP_CWORD=$((CURRENT-1)) _PROVISION_COMPLETE=zsh_complete provision)}")
...
compdef _provision_completion provision;
